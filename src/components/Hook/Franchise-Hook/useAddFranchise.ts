...
const useAddFranchise = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Append basic fields
    formPayload.append("BeneficiaryId", "");
    formPayload.append("Taluka", "");
    formPayload.append("Village", "");
    formPayload.append("District", "");
    formPayload.append("PumpCapacity", "");
    formPayload.append("RooftopPlantCapacity", "");
    formPayload.append("OffOnGrid", "");
    formPayload.append("SanctionLoad", "");
    formPayload.append("LoadExtension", "");
    formPayload.append("WaterSource", "");
    formPayload.append("SummerDepth", "");
    formPayload.append("WinterDepth", "");
    formPayload.append("PumpHead", "");
    formPayload.append("BgNumber", "");
    formPayload.append("BgValidity", "");
    formPayload.append("BgClaimPeriod", "");
    formPayload.append("InsuranceNumber", "");
    formPayload.append("InsuranceAgency", "");
    formPayload.append("InsuranceValidity", "");
    formPayload.append("InstallationVendor", "");
    formPayload.append("PumpHeadSelect", "0");
    formPayload.append("AcDc", "");
    formPayload.append("Surface", "");
  
    // Append formData fields
    formPayload.append("ShopName", formData.shopName || "");
    formPayload.append("Fname", formData.franchiseName || "");
    formPayload.append("Lattitude", formData.latitude || "");
    formPayload.append("Longitude", formData.longitude || "");
    formPayload.append("Details", formData.details || "");
    formPayload.append("Location", formData.shopLocation || "");
    formPayload.append("Phone", formData.mobileNo || "");
    formPayload.append("Phone2", formData.anotherMobileNo || "");
    formPayload.append("EmailId", formData.emailId || "");
    formPayload.append("SellAmt", String(formData.sellAmount || 0));
    formPayload.append("SellDate", formatDate(formData.sellDate));
    formPayload.append("TypeOfVendor", String(formData.franchiseType || 0));
    formPayload.append("AadharNo", String(formData.AdharNo || ""));
    formPayload.append("PanNo", formData.PanNo || "");
    formPayload.append("GstNo", formData.GSTINNo || "");
    formPayload.append("GumastaNo", formData.GumastaNo || "");
    formPayload.append("MsmeNo", formData.MSMENo || "");
    formPayload.append("BankName", formData.BankName || "");
    formPayload.append("AccountName", formData.BankHolderName || "");
    ...
    formPayload.append("UpiNo", formData.UPIID || "");
    formPayload.append("Status", String(formData.status || 0));
    formPayload.append("CustomerId", "F96673253");
    formPayload.append("StateId", String(formData.StateId || 1));
    formPayload.append("Roll", String(formData.roll || 5));
    formPayload.append("CreatedBy", "5");
    formPayload.append("ModifiedBy", "5");
    formPayload.append("CreatedDate", formatDate(new Date().toISOString()));
    formPayload.append("ModifiedDate", formatDate(new Date().toISOString()));
  
    // Append files only if they exist
    if (formData.FrontAdharcardPhoto)
      formPayload.append("AadharCard", formData.FrontAdharcardPhoto);
    if (formData.BackAdharcardPhoto)
      formPayload.append("AadharCard2", formData.BackAdharcardPhoto);
    if (formData.FrontPanPhoto)
      formPayload.append("PanCard", formData.FrontPanPhoto);
    if (formData.BackPanPhoto)
      formPayload.append("PanCard2", formData.BackPanPhoto);
    if (formData.gstCretificate)
      formPayload.append("GstCertificate", formData.gstCretificate);
    if (formData.GumastaCretificate)
      formPayload.append("Gumasta", formData.GumastaCretificate);
    if (formData.MSMECertificate)
      formPayload.append("Msme", formData.MSMECertificate);
    if (formData.UploadFoodLicense)
      formPayload.append("FoodLicence", formData.UploadFoodLicense);
    if (formData.UploadFoodLicenseReceipt)
      formPayload.append("FoodLicenceReceipt", formData.UploadFoodLicenseReceipt);
    if (formData.UploadAgreementCopy)
      formPayload.append("AgreementCopy", formData.UploadAgreementCopy);
  
    try {
      const response : any = await createFranchiseCreate(formPayload);
      if (response.ok) {
        setMessage("Franchise added successfully!");
      } else {
        setMessage("Error: Failed to add Franchise.");
