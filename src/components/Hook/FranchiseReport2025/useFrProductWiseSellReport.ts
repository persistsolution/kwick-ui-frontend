...
import { fetchProductWiseSellReportApi } from "../../api/SubFranchise-API/Report2025-Api/FrRawInventoryStockReportApi";
...
const useFrProductWiseSellReport = () => {
  ...
  const handelfetchProductWiseSellReport = async () => {
    ...
    setLoading(true)
    try {
      const response: any = await fetchProductWiseSellReportApi(Number(frId));
...
export default useFrProductWiseSellReport;
