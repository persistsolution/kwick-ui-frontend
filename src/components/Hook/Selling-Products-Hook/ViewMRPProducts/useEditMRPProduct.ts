...
const useEditMrpProductForm = () => {
  ...
  const handleFetchEditMrpProductData = async () => {
    try {
      const response: any = await fetchEditMRPProductsAPI(Number(id))
  ...
  const handelAddProduct = async () => {
    ...
    try {
      const response: any = await updateMRPProductsAPI(Number(id), productData)
...
export default useEditMrpProductForm;
