...
const useAddGodownReturnProduct = () => {
  ...
  const handlSelectGodownProductList = async (selectedOption: any) => {
    ...
    if (response) {
      setFormValues((prevState) => ({
        ...
        returnstockQty: prevState.returnstockQty,
        ...
        vendorsList: prevState.vendorsList,
  ...
  const handleChange = (e: any) => {
    ...
    if (type === "file") {
      ...
      if (name === "returnstockQty" || name === "productPrice") {
        const returnstockQty =
          name === "returnstockQty" ? Number(value) : Number(formValues.returnstockQty);
        const unitPrice =
          name === "productPrice"
            ? Number(value)
        ...
        updatedValues = {
          ...updatedValues,
          totalPrice: returnstockQty * unitPrice,
        };
      }
  ...
  const handelAddGodownStock = async () => {
    const totalQty = formValues.addGodownStockArray.reduce(
      (acc: number, item: any) => acc + Number(item.returnstockQty),
      0
    );
    ...
    const productdetailsList = formValues.addGodownStockArray.map(
      (item: any) => ({
        ...
        Qty: Number(item.returnstockQty),
    ...
    setFormValues((prevValues) => ({
      ...
      addGodownStockArray: [
        ...prevValues.addGodownStockArray,
        {
          ...
          returnstockQty: formValues.returnstockQty,
  ...
  const handelChangeAddedGodownPoduct = (e: any, idx: number) => {
    ...
    const newValue =
      name === "returnstockQty" || name === "productPrice" ? Number(value) : value;
    ...
    if (name === "returnstockQty" || name === "productPrice") {
      const returnstockQty =
        Number(updatedAddGodownStockArray[idx].returnstockQty) || 0;
      const productPrice =
        Number(updatedAddGodownStockArray[idx].productPrice) || 0;
      updatedAddGodownStockArray[idx].totalPrice = returnstockQty * productPrice;
    }
...
export default useAddGodownReturnProduct;
